	...
	render() {
		...
		return (
			<div className="item-wrapper">
				...
				>
					<img className={imgClass ? imgClass : undefined} src={getImage(type)} alt="ExplorerItem" />
					<span className={type === constants.FOLDER ? 'folder-name' : 'file-name'}>{name}</span>
					{type === constants.FILE ? <span className="extention">{`.${name.split('.').pop()}`}</span> : ''}
				</div>
			</div>
		);
	}
